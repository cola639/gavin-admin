...
public class EmailTestController {
  ...
  /** Preview HTML in browser. Example: /test/email/preview/html?template=test/welcome */
  ...
  public ResponseEntity<String> previewHtml(@RequestParam String template) {
    ...
    return ResponseEntity.ok(
        result.getHtml() == null ? "<!-- no html template -->" : result.getHtml());
  }

  /** Preview text. Example: /test/email/preview/text?template=test/welcome */
  ...
  public AjaxResult sendPersonalized(
      @Valid @RequestBody List<@Valid PersonalizedEmailSendRequest> requests) {
    var result = emailAppService.sendPersonalized(requests);
    return AjaxResult.success(result);
  }
}
