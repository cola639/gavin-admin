...
import com.api.system.service.email.EmailTemplateTestDataFactory;
...
public class EmailAppServiceImpl implements com.api.system.service.email.EmailAppService {
  ...
  private final EmailTemplateTestDataFactory testDataFactory;
  ...
  public BatchEmailSendResult sendBatch(BatchEmailSendRequest request) {
    ...
    if ((html == null || html.isBlank()) && (text == null || text.isBlank())) {
      throw new ServiceException(
          "No email body rendered. Please check template files (.html/.txt).");
    }
    ...
    log.info(
        "Email: requestId={}, totalRecipients={}, subject={}, template={}, mode={}, perCallLimit={}",
        requestId,
        recipients.size(),
        subject,
        template,
    ...
    for (var batch : batches) {
      ...
      try {
        ...
        log.error(
            "Email: batch send failed, requestId={}, batchSize={}", requestId, batch.size(), ex);
        batch.forEach(
            mail ->
                results.add(
                    EmailSendItemResult.builder()
  ...
  @Override
  public BatchEmailSendResult sendPersonalized(List<PersonalizedEmailSendRequest> requests) {
    var requestId = UUID.randomUUID().toString();

    if (requests == null || requests.isEmpty()) {
      throw new ServiceException("Personalized requests must not be empty.");
    }
    if (props.getFrom() == null || props.getFrom().isBlank()) {
      throw new ServiceException("Email 'from' address is not configured (app.email.from).");
    }
    ...
    log.info(
        "Email: personalized batch send start, requestId={}, items={}", requestId, requests.size());

    var results = new ArrayList<EmailSendItemResult>();

    for (var item : requests) {
      var recipient = safeTrim(item.getRecipient());
      var subject = safeTrim(item.getSubject());
      var template = safeTrim(item.getTemplate());
      var mode = item.getBodyMode() == null ? EmailBodyMode.BOTH : item.getBodyMode();
      ...
      try {
        var vars = mergeDefaultVars(item.getVariables(), template);
        var rendered = templateRenderer.render(template, vars);
        ...
        log.error(
            "Email: personalized send failed, requestId={}, recipient={}",
            requestId,
            recipient,
            ex);
        results.add(
  ...
  private EmailSendResponse sendWithRetry(EmailSendCommand cmd) {
    ...
    while (true) {
      try {
        ...
        log.warn(
            "Email: transient SES error, status={}, attempt={}, sleepMs={}, message={}",
            status,
            attempt,
            sleepMs,
            ex.getMessage());
