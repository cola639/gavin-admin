...
/**
 * Repository for SysUserPost entity. Provides data access methods for managing user-post
 * associations.
 */
@Repository
public interface SysUserPostRepository
    extends JpaRepository<SysUserPost, Long>, JpaSpecificationExecutor<SysUserPost> {

  /**
   * Delete user-post associations by userId.
   *
   * <p>Equivalent to: delete from sys_user_post where user_id = ?
   *
   ...
   *
   * <p>Equivalent to: select count(1) from sys_user_post where post_id = ?
   *
   ...
   *
   * <p>Equivalent to: delete from sys_user_post where user_id in (...)
   *
  ...
  /**
   * Save multiple user-post associations. (Replaces MyBatis batchUserPost).
   *
