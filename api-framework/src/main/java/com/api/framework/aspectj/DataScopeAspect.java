...
public class DataScopeAspect {
    ...
    private void handleDataScope(DataScope controllerDataScope) {
        ...
        String permission = StringUtils.defaultIfEmpty(
                ...
                DataScopeContextHolder.getContext()
        );

        // 这里你生成 spec 直接用，不要存到 PermissionContextHolder
        Specification<Object> spec = buildSpecification(currentUser, permission);

        log.debug("DataScope applied for userId={}, deptId={}, permission={}",
                currentUser.getUserId(), currentUser.getDeptId(), permission);
    }
            ...
            for (SysRole role : roles) {
                ...
                switch (role.getDataScope()) {
                    ...
                    case DATA_SCOPE_DEPT -> {
                        predicate = cb.or(predicate, cb.equal(root.join("dept", JoinType.LEFT).get("deptId"), user.getDeptId()));
                    }
                    ...
                    case DATA_SCOPE_CUSTOM -> {
                        predicate = cb.or(predicate, root.join("dept", JoinType.LEFT).get("deptId").in(role.getDeptIds())); // assume role.deptIds is available
                    }
                    default -> {
